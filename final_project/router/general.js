...
// const resListOfBooks = async (books, res) => {
//   try {
...
const resListOfBooks = async (res) => {
  try {
    res.send(JSON.stringify({books}, null, 4));
  } catch (error) {
    console.error(error.toString());
...
public_users.get('/',function (req, res) {
  ...
  resListOfBooks(res);
  return;// resListOfBooks(books,res);
  // return res.status(300).json({message: "Yet to be implemented"});
});

const resBookByISBN = async (req, res) => {
  try {
...
public_users.get('/isbn/:isbn',function (req, res) {
  ...
  // const isbn = req.params.isbn;
  // console.log('ISBN requested: ' + isbn);
  resBookByISBN(req, res);
  return; // res.send(books[isbn]);

  // return res.status(300).json({message: "Yet to be implemented"});
 });

 const resBookByAuthor = async (req, res) => {
  try {
...
public_users.get('/author/:author',function (req, res) {
  ...
  // // Filter the books array to find books which author matches the extracted author parameter
  // let filtered_author = Object.values(books).filter((book) => book.author === author);
  resBookByAuthor(req, res);
  return; //res.send(filtered_author);
  // return res.status(300).json({message: "Yet to be implemented"});
});


const resBookByTitle = async (req, res) => {
  try {
    // Extract the author parameter from the request URL
